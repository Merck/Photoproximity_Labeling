...
if(length(Arg)==0){
	...
	Special_List<-"CD45_Special_p70.txt"
}else{
	Name=Arg[1]
	Target=Arg[2]
	TargetName=Arg[3]
...
if(file.exists(paste0(WD,"data/",Special_List))){
	Special_List<-read.table(paste0(WD,"data/",Special_List),stringsAsFactors=FALSE,sep="\t")
}else{
	print("Special coloration file not found")
	Special_List<-NULL
}
...
#  Special coloring for proteins regardless of significance.  
Color<-ifelse(Data_0[,"Gene.Symbol"]%in%Special_List[,1],"Special",Color)
...
#  Put labels back in if special list is used.  
Keep<-which(Data_0$Gene.Symbol%in%Special_List[,1])
Labels_All[Keep]<-Data_0$Gene.Symbol[Keep]
...
#  Set colors
if(!is.null(Special_List)){
	Vals=c("Antibody"="#800000","Associated"="#68BC44","Enriched"="#BE29EC","NS"="black","Special"="#68BC44")
}else{
	Vals=c("Antibody"="#800000","Associated"="#68BC44","Enriched"="#BE29EC","NS"="black")
}
...
if(!is.null(Special_List)){
	Vals=c("Associated"="#68BC44","Enriched"="#BE29EC","NS"="black","Special"="#68BC44")
	BP<-2.5
	PP<-1.8
	Force=5
}else{
	Vals=c("Associated"="#68BC44","Enriched"="#BE29EC","NS"="black")
	BP<-0.3
	PP<-0.8
	Force=1
}
...
scale_x_continuous(breaks=c(-xlimf:xlimf))+
scale_y_continuous(breaks=c(0:ylim))+
geom_label_repel(size=12,aes(label=GeneLabels),color="black",box.padding=BP,point.padding=PP,segment.color="grey50",fill=NA,label.size=NA,family=FONT,fontface="bold",force=Force)+
theme_classic(base_size=50)+
theme(legend.position = "none",axis.title.y=element_text(family=FONT),axis.title.x=element_text(family=FONT))+
guides(fill=guide_legend(title=""))
